...
def detect_macd_signal(k, symbol,interval,ema_fast, ema_slow):
    ...
    data['cross_macd_long'] = np.where(
        (signal.shift(-3) > macd.shift(-3)) & (macd > signal) & (macd.shift(-13) < 0),
        1, 0
    )
    ...
    data['cross_macd_short'] = np.where(
        (macd.shift(-3) > signal.shift(-3)) & (signal > macd) & (macd.shift(-13) > 0),
        1, 0
    )
    ...
    print(data[['Time', 'Volume', 'Close', 'macd', 'signal', 'cross_macd_long', 'cross_macd_short', 'macd_slope', 'volume_slope']].tail(60))


    # # 🚀 Detectar si hay señal en el índice `k`
    # if cross_long[k] and slope_long[k]:
    #     return "LONG"
    # elif cross_short[k] and slope_short[k]:
    #     return "SHORT"
    # else:
    #     return None
...
detect_macd_signal(201, 'BTCUSD', 15, 10, 50)  
